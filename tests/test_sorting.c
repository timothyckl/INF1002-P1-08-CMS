...
void test_sort_records_single_record(void) {
    ...
    sort_records(table->records, table->record_count, SORT_FIELD_ID, SORT_ORDER_ASC);

    ASSERT_EQUAL_INT(original.id, table->records[0].id, "Single record should remain unchanged");
...
void test_sort_records_by_id_ascending(void) {
    ...
    for (int i = 0; i < 5; i++) {
        StudentRecord record = create_test_record(ids[i], "Test", "Programme", 75.0f);
    ...
    for (size_t i = 1; i < table->record_count; i++) {
        ASSERT_TRUE(table->records[i-1].id < table->records[i].id, "Records should be in ascending ID order");
    }
...
void test_sort_records_by_id_descending(void) {
    ...
    sort_records(table->records, table->record_count, SORT_FIELD_ID, SORT_ORDER_DESC);

    // verify descending order
    for (size_t i = 1; i < table->record_count; i++) {
        ASSERT_TRUE(table->records[i-1].id > table->records[i].id, "Records should be in descending ID order");
    }
...
void test_sort_records_by_mark_ascending(void) {
    ...
    for (int i = 0; i < 5; i++) {
        StudentRecord record = create_test_record(1000 + i, "Test", "Programme", marks[i]);
        table_add_record(table, &record);
    }

    sort_records(table->records, table->record_count, SORT_FIELD_MARK, SORT_ORDER_ASC);

    // verify ascending order by mark
    for (size_t i = 1; i < table->record_count; i++) {
        ASSERT_TRUE(table->records[i-1].mark <= table->records[i].mark, "Records should be in ascending mark order");
    }
...
void test_sort_records_by_mark_descending(void) {
    ...
    sort_records(table->records, table->record_count, SORT_FIELD_MARK, SORT_ORDER_DESC);

    // verify descending order by mark
    for (size_t i = 1; i < table->record_count; i++) {
        ASSERT_TRUE(table->records[i-1].mark >= table->records[i].mark, "Records should be in descending mark order");
    }
...
void test_sort_records_already_sorted(void) {
    ...
    for (int i = 0; i < 5; i++) {
        StudentRecord record = create_test_record(1000 + i, "Test", "Programme", 50.0f + i);
        table_add_record(table, &record);
    }
    ...
    for (size_t i = 1; i < table->record_count; i++) {
        ASSERT_TRUE(table->records[i-1].id < table->records[i].id, "Already sorted records should remain sorted");
    }
...
void test_sort_records_reverse_sorted(void) {
    ...
    for (int i = 0; i < 5; i++) {
        StudentRecord record = create_test_record(1004 - i, "Test", "Programme", 75.0f);
        table_add_record(table, &record);
    }
    ...
    for (size_t i = 1; i < table->record_count; i++) {
        ASSERT_TRUE(table->records[i-1].id < table->records[i].id, "Reverse sorted records should be sorted");
    }
...
void test_sort_records_duplicate_marks_tiebreaker(void) {
    ...
    for (size_t i = 1; i < table->record_count; i++) {
        if (table->records[i-1].mark == table->records[i].mark) {
            ASSERT_TRUE(table->records[i-1].id < table->records[i].id, "Duplicate marks should be tie-broken by ID");
        }
    }
...
void test_sort_records_all_same_values(void) {
    ...
    for (size_t i = 1; i < table->record_count; i++) {
        ASSERT_TRUE(table->records[i-1].id <= table->records[i].id, "Same marks should maintain or sort by ID");
    }
...
void test_sort_records_boundary_marks(void) {
    ...
    ASSERT_EQUAL_FLOAT(0.0f, table->records[0].mark, 0.01f, "Minimum mark should be first");
    ASSERT_EQUAL_FLOAT(100.0f, table->records[2].mark, 0.01f, "Maximum mark should be last");
...
void test_sort_records_large_dataset(void) {
    ...
    // verify sorted
    for (size_t i = 1; i < table->record_count; i++) {
        ASSERT_TRUE(table->records[i-1].id <= table->records[i].id, "Large dataset should be sorted correctly");
    }
