...
void test_log_event_first_event(void) {
    ...
    if (log->count > 0) {
        ASSERT_EQUAL_INT(OPEN, log->entries[0].operation, "Operation should be OPEN");
        ASSERT_EQUAL_INT(OP_SUCCESS, log->entries[0].status, "Status should be OP_SUCCESS");
    }
...
void test_log_event_different_operations(void) {
    ...
    ASSERT_EQUAL_INT(10, log->count, "Count should be 10 for different operations");
    ASSERT_EQUAL_INT(OPEN, log->entries[0].operation, "First operation should be OPEN");
    ASSERT_EQUAL_INT(STATISTICS, log->entries[9].operation, "Last operation should be STATISTICS");
...
void test_log_event_different_statuses(void) {
    ...
    ASSERT_EQUAL_INT(OP_SUCCESS, log->entries[0].status, "First status should be OP_SUCCESS");
    ASSERT_EQUAL_INT(OP_ERROR_GENERAL, log->entries[5].status, "Last status should be OP_ERROR_GENERAL");
